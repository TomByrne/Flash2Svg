...
(function(ext) {
  ...
  Bezier.prototype = {
    ...
    doesIntersect: function(curve, acceptEndsMeet) {
      if(acceptEndsMeet == undefined)acceptEndsMeet = true;

      var intersects = this.intersects(curve);
      if(intersects && intersects.length){
        if(acceptEndsMeet)return true;

        var p1 = curve.p1 || curve.points[0];
        var p2 = curve.p2 || curve.points[curve.points.length-1];
        var s1 = this.points[0];
        var s2 = this.points[this.points.length-1];

        var match1 = (p1.x == s1.x && p1.y == s1.y) || (p1.x == s2.x && p1.y == s2.y);
        var match2 = (p2.x == s2.x && p2.y == s2.y) || (p2.x == s1.x && p2.y == s1.y);

        fl.trace("p1: "+p1.x+" "+p1.y+" - "+p2.x+" "+p2.y);
        fl.trace("s1: "+s1.x+" "+s1.y+" - "+s2.x+" "+s2.y);
        fl.trace("match: "+match1+" "+match2+" "+intersects.length);

        if(match1){
          if(match2) return intersects.length > 2;
          else return intersects.length > 1;
        }else if(match2){
    ...
    lineIntersects: function(line) {
      ...
      return utils.roots(this.points, line).filter(function(t) {
        var p = self.get(t);
        return (utils.between(p.x, mx, MX) || mx==MX) && (utils.between(p.y, my, MY) || my==MY);
      });
    },
